"""
Analytics for the increment (delta) CNP experiment.

Run from repo root:
    .venv/bin/python3 -m neural_processes.examples.vol_surface_increment.analytics
    .venv/bin/python3 -m neural_processes.examples.vol_surface_increment.analytics --out_dir results/vol_surface_increment
"""
...
def eval_rmse_vs_ctx(model: FittedDeltaCNP, dataset, indices, ctx_sizes, n_reps=8):
    ...
    prior_valid  = valid_tgt & np.isfinite(prior_tgt)
...
def eval_zeroshot_delta(model: FittedDeltaCNP, dataset, indices, n_ctx=50, n_reps=10):
    ...
    for excl in range(n_assets):
        mask = (aid_tgt == excl) & valid_tgt & np.isfinite(prior_tgt)
        if not mask.any():
            continue

        # prior-only: prior IV directly, no model call
        prior_err2 = (prior_tgt - true_tgt) ** 2
        ...
        sq_bl, sq_zs = [], []
        for _ in range(n_reps):
            # baseline: random n_ctx from all assets
            perm    = rng.permutation(ctx_max)[:n_ctx]
            ...
            # zero-shot: exclude asset excl from context
...
def plot_rmse_vs_ctx(results, out_path=None):
    ...
    ax.set_title("RMSE vs context size — increment model")
    if out_path:
...
def plot_per_maturity_rmse(model_rmse, prior_rmse, n_ctx=100, out_path=None):
    ...
    ax.set_title(f"Per-maturity RMSE — increment model  (n_ctx={n_ctx})")
    ax.legend()
    fig.tight_layout()
    if out_path:
        fig.savefig(out_path, dpi=120, bbox_inches="tight")
...
def plot_zeroshot_increment(results, asset_names=None, n_ctx=50, out_path=None):
    ...
    ax.set_title("Zero-Shot Asset Reconstruction — increment model")
    ax.legend()
    fig.tight_layout()
    if out_path:
        fig.savefig(out_path, dpi=120, bbox_inches="tight")
...
def plot_reconstruction(model: FittedDeltaCNP, dataset, day_idx,
    ...
    """
    Reconstruction plot for a single day.
    Shows true IV (dashed), prior IV (dotted), model prediction (solid).
    """
    ctx_max     = dataset.ctx_max
    n_assets    = dataset.n_assets
...
def main(cfg: Config, model=None, dataset=None):
    ...
    plot_reconstruction(model, dataset, last_val_day,
