...
class Experiment:
    """
    Two-phase experiment framework: train to learn edges, then run sequentially
    ...
    def run(self, dates: list[pd.Timestamp]) -> list[Any]:
        """
        Sequential evaluation over a list of dates.

        dates[0]:  perfect fit via build_graph (all available data, no split).
        dates[1:]: sparse update — observations are split; train split feeds the
                   updater; test split is held out for output_fn evaluation.

        Returns a list of output_fn results, one per update step (len = len(dates) - 1).
        """
        if len(dates) < 2:
        ...
        for i in range(1, len(dates)):
            ...
            obs_full = self.build_obs(self.df, curr_date)
            train_obs, test_obs = self.splitter.split(obs_full)
            ...
            for model in tqdm(self.models):
                edges = self._edges[model.name]
                prior = current_graphs[model.name]
            ...
            results.append(self.output_fn(fitted_graphs, train_obs, test_obs))
