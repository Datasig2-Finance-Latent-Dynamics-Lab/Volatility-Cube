...
class BSplineUpdater:
    ...
    def update(
        ...
        # Precompute prior parameters and precision matrices.
        # Precision is read from the initial graph (x0 = rolled prior), which carries
        # the decayed posterior precision from the previous day's fit. This makes the
        # temporal pull proportional to how well each node was observed before.
        prior_params: dict = {}
        prior_precs: dict = {}
        if prior_graph is not None:
            for nid_ in node_ids:
                try:
                    ...
                    pass
                prec = graph.get(nid_).precision  # graph is x0 = rolled prior
                prior_precs[nid_] = (
                    float(prec) * np.eye(p) if isinstance(prec, (int, float)) else prec.copy()
                )
        ...
        for i, nid in enumerate(node_ids):
            ...
            if nid in prior_params and self.lambda_temporal > 0:
                ...
                P_t = prior_precs.get(nid, np.eye(p))
                H[sl, sl] += 2.0 * self.lambda_temporal * P_t
                b[sl]     += 2.0 * self.lambda_temporal * (P_t @ c_prior_i)
