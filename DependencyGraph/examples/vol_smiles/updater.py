...
# Done by CaludeCode, doing normal minimization from scipy is painfully slow as one would expect.

# TODO: I will look more into this when I make states that are parametrized by a basis in general.
...
class BSplineUpdater:
    ...
    def update(
        ...
        for i, nid in enumerate(node_ids):
            ...
            # Penalises ||(c_i - c_prior_i) - (c_j - c_prior_j)||^2 when a prior
            # is available, so dependencies act on *changes* not absolute levels.
            # The Hessian structure is identical to the absolute case; only the
            # RHS gains a correction from the prior deltas.
            if self.lambda_graph > 0:
                for j, nid_j in enumerate(node_ids):
                    if i == j:
                        continue
                    w_ij = graph.dependencies.get_edge(nid, nid_j)
                    if w_ij is None:
                        continue
                    sl_j = slice(j * p, (j + 1) * p)
                    H[sl, sl]   += 2.0 * self.lambda_graph * float(w_ij) * np.eye(p)
                    H[sl, sl_j] -= 2.0 * self.lambda_graph * float(w_ij) * np.eye(p)
                    if nid in prior_params and nid_j in prior_params:
                        b[sl] += (
                            2.0 * self.lambda_graph * float(w_ij)
                            * (prior_params[nid] - prior_params[nid_j])
                        )
        ...
        for i, nid in enumerate(node_ids):
            sl = slice(i * p, (i + 1) * p)
            new_nodes[nid] = graph.get(nid).from_parameters(c_flat[sl])

        return Graph(observations.date, new_nodes, graph.dependencies)
